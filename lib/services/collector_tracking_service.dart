...
class CollectorTrackingService {
  ...
  Future<Map<String, dynamic>> getTodayTrackingData() async {
    ...
    try {
      ...
      final distance = (data['totalDistance'] ?? 0.0) as double;
      
      Duration duration;
      if (data['status'] == 'active' && data['startTime'] != null) {
        final startTime = (data['startTime'] as Timestamp).toDate();
        duration = DateTime.now().difference(startTime);
      } else if (data['durationSeconds'] != null) {
      ...
      final completionRate = totalGarbage > 0 
          ? (collectedGarbage / totalGarbage * 100) 
          : 0.0;
